...
- (YPCDVAudioFile*)audioFileForResource:(NSString*)resourcePath withId:(NSString*)mediaId doValidation:(BOOL)bValidate forRecording:(BOOL)bRecord
{
    ...
    if (audioFile == nil) {
        ...
        if ((resourcePath == nil) || ![resourcePath isKindOfClass:[NSString class]] || [resourcePath isEqualToString:@""]) {
            ...
            audioFile = [[YPCDVAudioFile alloc] init];
    ...
    if (bError) {
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:errcode message:errMsg]];
...
- (void)create:(CDVInvokedUrlCommand*)command
{
    ...
    YPCDVAudioFile* audioFile = [self audioFileForResource:resourcePath withId:mediaId doValidation:NO forRecording:NO];

    if (audioFile == nil) {
        NSString* errorMessage = [NSString stringWithFormat:@"Failed to initialize Media file with path %@", resourcePath];
        NSString* jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_ABORTED message:errorMessage]];
        [self.commandDelegate evalJs:jsString];
    } else {
...
#pragma unused(callbackId)
    ...
    if ([self soundCache] != nil) {
        YPCDVAudioFile* audioFile = [[self soundCache] objectForKey:mediaId];
        if (audioFile != nil) {
...
#pragma unused(callbackId)
    ...
    YPCDVAudioFile* audioFile = [self audioFileForResource:resourcePath withId:mediaId doValidation:YES forRecording:NO];
    if ((audioFile != nil) && (audioFile.resourceURL != nil)) {
        ...
        if (!bError) {
            ...
            if (!bError) {
                ...
                jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%.3f);\n%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_DURATION, position, @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
                [self.commandDelegate evalJs:jsString];
            }
        }
        if (bError) {
            ...
            // jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR,  MEDIA_ERR_NONE_SUPPORTED];
            jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_NONE_SUPPORTED message:nil]];
            [self.commandDelegate evalJs:jsString];
        }
    }
...
- (BOOL)prepareToPlay:(YPCDVAudioFile*)audioFile withId:(NSString*)mediaId
{
    ...
    if ([resourceURL isFileURL]) {
        audioFile.player = [[YPCDVAudioPlayer alloc] initWithContentsOfURL:resourceURL error:&playerError];
    } else {
        ...
        if (playerError) {
            ...
            audioFile.player = [[YPCDVAudioPlayer alloc] initWithContentsOfURL:fileURL error:&playerError];
        }
    }
...
- (void)stopPlayingAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        [audioFile.player stop];
        audioFile.player.currentTime = 0;
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_STATE, MEDIA_STOPPED];
...
- (void)pausePlayingAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_STATE, MEDIA_PAUSED];
    }
    // ignore if no media playing
...
- (void)seekToAudio:(CDVInvokedUrlCommand*)command
{
    ...
    if ((audioFile != nil) && (audioFile.player != nil)) {
        ...
        if (posInSeconds >= audioFile.player.duration) {
            ...
            [audioFile.player stop];
            audioFile.player.currentTime = 0;
            jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%.3f);\n%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_POSITION, 0.0, @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_STATE, MEDIA_STOPPED];
            // NSLog(@"seekToEndJsString=%@",jsString);
        } else {
            audioFile.player.currentTime = posInSeconds;
            jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%f);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_POSITION, posInSeconds];
...
#pragma unused(mediaId)
    ...
    NSString* jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%.3f);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_POSITION, position];
    [self.commandDelegate evalJs:jsString];
...
#pragma unused(callbackId)
    ...
    NSString* mediaId = [command argumentAtIndex:0];
    YPCDVAudioFile* audioFile = [self audioFileForResource:[command argumentAtIndex:1] withId:mediaId doValidation:YES forRecording:YES];
        ...
        void (^startRecording)(void) = ^{
            ...
            if ([self hasAudioSession]) {
                ...
                if (![self.avSession setActive:YES error:&error]) {
                    ...
                    // jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR, MEDIA_ERR_ABORTED];
                    jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_ABORTED message:errorMsg]];
                    [self.commandDelegate evalJs:jsString];
                    return;
                }
            }
            ...
            audioFile.recorder = [[YPCDVAudioRecorder alloc] initWithURL:audioFile.resourceURL settings:recorderSettingsDict error:&error];
            ...
            if (error == nil) {
                ...
                if (recordingSuccess) {
                    ...
                    jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_STATE, MEDIA_RUNNING];
                    [self.commandDelegate evalJs:jsString];
                }
            }
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
            [self.avSession performSelector:rrpSel withObject:^(BOOL granted){
                if (granted) {
                    ...
                    jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_ABORTED message:msg]];
                    [self.commandDelegate evalJs:jsString];
                }
            }];
...
{
    YPCDVAudioRecorder* aRecorder = (YPCDVAudioRecorder*)recorder;
    ...
    if (flag) {
        ...
        // jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%d);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR, MEDIA_ERR_DECODE];
        jsString = [NSString stringWithFormat:@"%@(\"%@\",%d,%@);", @"cordova.require('cordova-plugin-media-m4a.YPMedia').onStatus", mediaId, MEDIA_ERROR, [self createMediaErrorWithCode:MEDIA_ERR_DECODE message:nil]];
    }
    if (self.avSession) {
        [self.avSession setActive:NO error:nil];
    }
...
{
    YPCDVAudioPlayer* aPlayer = (YPCDVAudioPlayer*)player;
...
- (void)onReset
{
    for (YPCDVAudioFile* audioFile in [[self soundCache] allValues]) {
        if (audioFile != nil) {
            if (audioFile.player != nil) {
                [audioFile.player stop];
                audioFile.player.currentTime = 0;
...
@end
@implementation YPCDVAudioPlayer
@synthesize mediaId;

@end

@implementation YPCDVAudioRecorder
@synthesize mediaId;
